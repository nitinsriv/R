...
# plot Residuals vs Fitted
op <- par(mfrow = c(2, 2))
plot(ky_cam_access_model6)
par(op)

# Predict the house prices in the testing dataset
Predict_1 <- predict(ky_cam_access_model6,test[,-1])
test$ky_predicted_sale <- Predict_1


# Calculate RMSE: 1588457 on train data
ky_RSS_train <- c(crossprod(ky_cam_access_model6$residuals))
ky_RSS_train

ky_MSE_train <- ky_RSS_train / length(ky_cam_access_model6$residuals)
ky_MSE_train
...
dis_cam_access_model13 <- lm(Sales ~ PromotionWeek + NPS +  sc_Sponsorship ,na.action = na.exclude, data=dn_train)
