...
data_dir <- "data/"
data_file_comp <- "companies.txt"
data_file_rounds <- "rounds2.csv"
data_file_category_mapping <- "mapping.csv"
data_file_english_mapping <- "eng_countries.csv"


# construct data file path from config file
abs_datafile_path_comp <- paste(base_dir,data_dir,data_file_comp, sep="")
abs_datafile_path_rounds <- paste(base_dir,data_dir,data_file_rounds, sep="")
abs_datafile_path_categ_mapping <- paste(base_dir,data_dir,data_file_category_mapping, sep="")
abs_datafile_path_eng_mapping <- paste(base_dir,data_dir,data_file_english_mapping, sep="")


# Load the companies and rounds data into two data frames 
# and name them companies and rounds2 respectively.
comp <- read.delim(abs_datafile_path_comp,header=TRUE,fill=TRUE,stringsAsFactors = F)
rounds <- read.csv(abs_datafile_path_rounds,header=TRUE,stringsAsFactors = F)
categ_mapping <- read.csv(abs_datafile_path_categ_mapping,header=TRUE,stringsAsFactors = F)
eng_mapping <- read.csv(abs_datafile_path_eng_mapping,header=TRUE,stringsAsFactors = F)
