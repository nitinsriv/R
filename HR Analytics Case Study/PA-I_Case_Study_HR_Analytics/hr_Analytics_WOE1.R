...
# remove TotalWorkingYears10.12 since it is related to TotalWorkingYears17.22
model_28 <- glm(Attrition ~ EnvironmentSatisfaction0.56 + JobSatisfaction0.43 + WorkLifeBalance0.87 + 
                  BusinessTravelTravel_Frequently + 
                  JobLevel2 + JobRoleManufacturing.Director + JobRoleResearch.Director + 
                  MaritalStatusSingle +  NumCompaniesWorked0.36 + TrainingTimesLastYear6 + 
                  JobInvolvement3 +  
...
# remove YearsWithCurrManager3 since it became insignificant
model_29 <- glm(Attrition ~ EnvironmentSatisfaction0.56 + JobSatisfaction0.43 + WorkLifeBalance0.87 + 
                  BusinessTravelTravel_Frequently + JobLevel2 + JobRoleManufacturing.Director + 
                  JobRoleResearch.Director + MaritalStatusSingle +  NumCompaniesWorked0.36 + 
                  TrainingTimesLastYear6 + JobInvolvement3 +TotalWorkingYears17.22 + YearsAtCompany5.8 +
                  YearsWithCurrManager1.2 + Age38. + ActualWorkingHours,
                family = "binomial", data = train)
summary(model_29)
vif(model_29)

# remove TotalWorkingYears17.22 since it Age38.
model_30 <- glm(Attrition ~ EnvironmentSatisfaction0.56 + JobSatisfaction0.43 + WorkLifeBalance0.87 + 
                  BusinessTravelTravel_Frequently + JobLevel2 + JobRoleManufacturing.Director + 
                  JobRoleResearch.Director + MaritalStatusSingle +  NumCompaniesWorked0.36 + 
                  TrainingTimesLastYear6 + 
                  JobInvolvement3 + YearsAtCompany5.8+YearsWithCurrManager1.2 + 
                  Age38. + ActualWorkingHours,
                family = "binomial", data = train)
summary(model_30)
vif(model_30)

# Variables affecting attrition of an employee:
...
prob_seq = seq(.005,.90,length=100)
OUT = matrix(0,100,3)
for(i in 1:100)
{
...
# probability greaer than .18580 is 1 (employee will leave)
test_pred_attrition <- factor(ifelse(test_pred >= 0.18580, 1,0))


# confusion matrix with cutoff value equal 0.18580
...
# draw lift chart
ggplot(gainchart, aes(Decile)) + geom_line(aes(y = 1, colour = "Random Model")) + 
  geom_line(aes(y = lift, colour = "lift")) +geom_point(aes(y=1)) + geom_point(aes(y = lift, colour = "lift")) + 
  labs(x = "Decile", y = "Lift", title = "Lift chart")
