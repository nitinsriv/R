...
int_rate_plot <- ggplot(loan_raw, aes(x = loan_raw$loan_status, fill=loan_raw$loan_status))
label_int_rate <- labs(x = "Loan Status", y = "Number of Borrowers", title = "Loan Status", fill = "Loan Status ")
int_rate_plot_final <- int_rate_plot + label_int_rate + geom_histogram(stat="count")
#------------------------------------------------------------------------#
...
# Below plot shoes that for overall population grade B has more defaulters.
grade_status_count_plot <- ggplot(loan_raw,aes(loan_raw$grade,fill=loan_raw$loan_status))+geom_bar(position="dodge")
labelled_grade_status_count_plot <- grade_status_count_plot + labs(x="Grade",y="No. of Borrowers",title="Grade-Status Distrubution",fill="Status")
#--------------------------Segmented Analysis----------------------------------------#
...
summary_countwise_plot_final <- summary_countwise_plot + geom_point(alpha = 0.7,position = position_jitter(width = 0.4)) + change_shape + add_labels_1 + legend_position
...
#Creating the final plot#
summary_amountwise_plot_final <- summary_amountwise_plot + geom_point(alpha = 0.7,position = position_jitter(width = 0.4)) + change_shape + add_labels_2 + legend_position
...
#Creating the final plot#
summary_termwise_plot_final <- summary_termwise_plot + geom_point(alpha = 0.7,position = position_jitter(width = 0.4)) + change_shape + add_labels_3 + legend_position
...
#Creating the final plot#
summary_emplengthwise_plot_final <- summary_emplengthwise_plot + geom_point(alpha = 0.7,position = position_jitter(width = 0.35)) + change_shape + add_labels_4 + legend_position
...
#Creating the final plot#
summary_houseownershipwise_plot_final <- summary_houseownershipwise_plot + geom_point(alpha = 0.7,position = position_jitter(width = 0.4)) + change_shape + add_labels_5 + legend_position
...
#Creating the final plot#
summary_purposewise_plot_final <- summary_purposewise_plot + geom_point(alpha = 0.7,position = position_jitter(width = 0.35)) + change_shape + add_labels_6 + legend_position

# Default borrowers with no public records are given loans most.
pub_rec_plot <-ggplot(loan_chargedOff,aes(loan_chargedOff$pub_rec,fill= factor(loan_chargedOff$pub_rec)))+ geom_bar()+labs(x="Open Acc",y="No.of borrowers",title="Open Acc Vs Delinquency",fill="No. of Public Records")
...
# No. of defaulters are more who have 0 records
delq_pubrec_plot <- ggplot(loan_chargedOff,aes(loan_chargedOff$delinq_2yrs,fill=factor(loan_chargedOff$pub_rec)))+ geom_bar(position="dodge")+labs(x="Delinquency After 2 years",y="No.of Defaulters",title="Delinquency after two years vs No. of Defaulters")
...
Summary_openacc_plot <- Summary_openacc + add_labels_7
...
summary_fundedamnt_open_acc_chargeoff <- ggplot(loan_chargedOff, aes(x = funded_amnt, fill = factor(open_acc)))
...
add_labels_11 <- labs(title = "Frequency of Delinquent & Interest rate (COUNTWISE)", x = "Interest rate", y = "Count of loans")
